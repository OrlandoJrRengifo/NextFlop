...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "@/media/components/ui/popover";
...
import {
  ...
  DrawerTitle,
  DrawerTrigger,
} from "@/media/components/ui/drawer";
...
import { Button } from "@/media/components/ui/button";

import { getDominantColor } from "@/media/lib/Color";
import { getTextColorForBackground } from "@/media/lib/TextColor";
import movieRatings from "@/media/lib/Rating";
...
export default function MovieDetails({ id }) {
  ...
  useEffect(() => {
    // Scroll al inicio cuando cambia la película
    window.scrollTo(0, 0);
  }, [id]);
  ...
  useEffect(() => {
    ...
    const fetchMovieData = async () => {
      try {
        ...
        // Combinar crew importante
        const combineJobsById = (crewArray) => {
          const importantJobs = [
            "Director",
            "Screenplay",
            "Producer",
        ...
        const originCode =
          Array.isArray(movieRes.origin_country) &&
          movieRes.origin_country.length > 0
            ? movieRes.origin_country[0]
            : "US";

        const ratingEntry =
          ratingRes.results?.find((entry) => entry.iso_3166_1 === originCode) ||
          ratingRes.results?.find((entry) => entry.iso_3166_1 === "US");

        if (ratingEntry) {
          const certification = ratingEntry.release_dates.find((release) =>
        ...
        // Color dominante del poster
        if (movieRes.poster_path) {
  ...
  const handleShare = async () => {
    if (!movie) {
      toast.warning("No movie data available to share.");
      return;
    }

    try {
      const details = `${movie.title} (${movie.release_date}): ${
        movie.overview
  ...
  const handleAddPlayList = (movieId) => {
    ...
    if (playlist.includes(movieId)) {
      toast("Movie already in favorite.", {
        ...
        action: {
          label: "View",
          onClick: () => router.push("/reactflix/watchlist"),
        },
      });
      return;
    }
      ...
      >
        ...
        <div className="relative z-10 flex flex-col gap-3">
          <div className="flex flex-col">
            ...
            <p className="text-sm lg:text-base">
              <span className="px-1 border-black border border-opacity-10 bg-black bg-opacity-5 mr-1">
                <Popover>
                  <PopoverTrigger>{rating || "NR"}</PopoverTrigger>
                  <PopoverContent>
                    {movieRatings[rating] || "Not rated"}
                  </PopoverContent>
                </Popover>
              </span>
          ...
          <div className="flex flex-wrap items-start gap-y-1 py-4 px-2 mt-2 rounded-lg backdrop-blur-lg bg-black bg-opacity-5">
            {Object.entries(crew).map(([crewId, { name, jobs }]) => (
              <div
                key={crewId}
                className="flex flex-col items-start text-start w-1/2 lg:w-1/3 xl:w-1/4"
              >
                <Link href={`/reactflix/person/${crewId}`}>
